 ...
 */
export default function extractTagsProcessor(log, parseTagsProcessor, createDocMessage) {
	return {
		...
		$process(docs) {
			const tagExtractor = createTagExtractor(
	...
	function readTags(doc, docProperty, tagDef, tags) {
		...
		if (tagDef.multi) {
			...
			if (tags.length > 1) {
				throw new Error(
					createDocMessage(
						`Only one of "${tagDef.name}" (or its aliases) allowed. There were ${tags.length}`,
						doc,
					),
				);
			}
