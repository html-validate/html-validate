module.exports = function inlineValidationInlineTagDef(validateMap, createDocMessage) {
	...
	function handler(doc, tagName, description) {
		...
		if (!validation) {
			throw new Error(createDocMessage(`No inline validation exists with id "${description}".`, doc));
		}
		if (!validation.inlineValidationDoc) {
			throw new Error(createDocMessage(`Validation "${description}" does not have an associated inlineValidationDoc. Are you missing a processor (validations-generate)?"`, doc));
		}
